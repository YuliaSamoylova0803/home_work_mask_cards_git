...
def search_operation_by_str(operations: list[dict[str | Any]], string_search: str) -> list[dict[str | Any]]:
    """Функция для поиска в списке словарей операций по заданной строке — описанию с использованием библиотеки
    re. Функция для поиска в списке словарей операций по заданной строке принимает
    два аргумента: список с транзакциями и строку для поиска"""
    search_operation = [operation for operation in operations if re.findall(string_search, operation["description"])]
...
def counter_descriptions(operations: list[dict[str|Any]], categories: list[str]):
